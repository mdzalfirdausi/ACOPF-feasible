#!/usr/bin/env python3
"""
Chunked Ablated-PINN warm-start evaluation for:

    pglib_opf_case162_ieee_dtc

This script:
  1. Loads the existing 162-bus dataset:
         dataset/pglib_opf_case162_ieee_dtc_10000.pt

  2. Loads the EXISTING non-chunked cold-IPOPT reference:
         result/warmstart_cold_case162.csv

  3. Selects only the Instance_IDs belonging to this array chunk.

  4. Evaluates ONLY the five Ablated PINN checkpoints:
         model/162/*ablation_model*.pth

  5. Uses predicted pg/qg only as IPOPT initialization.
     pg/qg are clipped to generator limits, matching the main
     warm-start experiment.

  6. Does NOT recompute cold IPOPT.

  7. Saves separate chunked ablation outputs:
         warmstart_ablation_raw_case162_chunkXXX.csv
         warmstart_ablation_runlevel_case162_chunkXXX.csv
         warmstart_ablation_summary_case162_chunkXXX.csv
"""
...
DATASET_PATH = (
    f"./dataset/{CASE_NAME}_{TOTAL_SAMPLES}.pt"
)
...
def reconstruct_qcqp_matrices(problem):
    """
    Reconstruct the exact matrix representation used by the
...
def set_nn_dispatch_start(
    ...
    """
    Same IPOPT initialization convention as the main warm-start study.

    - NN pg/qg are clipped to generator bounds.
    - NN voltage is deliberately not used.
    - Voltage receives the same flat initialization.
    """
...
def main():
    ...
    print(
        "162-BUS ABLATED-PINN WARM-START EVALUATION"
    )
    print(
        "============================================================"
    )
    ...
    print(
        f"Start idx  : {args.start_idx}"
    )
    print(
        f"End idx    : {args.end_idx}"
    )
    print(
    ...
    if nbus != BUS_NUMBER:
        ...
        raise ValueError(
            f"Expected {BUS_NUMBER} buses, "
            f"but dataset reports {nbus}."
        )
    ...
    # ========================================================
    # Recover the same 1,000 held-out test instances
    # ========================================================
    ...
    if n_test != 1000:
        ...
        print(
            f"WARNING: Expected 1000 "
            f"held-out instances, "
            f"found {n_test}."
        )
    ...
    #
    # No cold IPOPT solve occurs in this script.
    # ========================================================
    ...
    print(
        "LOADING EXISTING 162-BUS COLD IPOPT BASELINE"
    )
    ...
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            f"Cold reference not found: "
            f"{COLD_FILE}"
        )

    cold_df_all = pd.read_csv(
        COLD_FILE
    )
    ...
    # --------------------------------------------------------
    # Select ONLY the rows belonging to this array task
    # --------------------------------------------------------
    ...
    # ========================================================
    # Apply EXACTLY the same cold-feasible mask
    # ========================================================
    ...
    # ========================================================
    # Find the five Ablated PINN checkpoints
    # ========================================================
