...
class baselineQCQPMLP(nn.Module):
    ...
    def forward(self, Pd: torch.Tensor, Qd: torch.Tensor, problem: dict) -> tuple:
        ...
        if self.projection_mode in ("voltage", "full"):
        ...
        # Reference-bus constraint is retained in ALL configurations
        v = v.clone()
        v[:, self.slack_imag_idx] = 0.0
        ...
        if self.projection_mode in ("generation", "full"):
...
if __name__ == "__main__":
    ...
    type=str,
    default="full",
    choices=["none", "voltage", "generation", "full"],
    )
    parser.add_argument(
        "--weights",
        type=str,
    ...
    loss_weights = WEIGHT_CONFIGS[args.weights]
