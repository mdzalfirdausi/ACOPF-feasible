...
Modified: Branched Neural Network Architecture for Primal and Dual Variables.
...
# --- MODEL DEFINITION ---
class BranchedHardKKT_QCQPMLP(nn.Module):
    """
    Version: Separated Neural Networks for primals (v, pg, qg) and duals.
    Input:
        Pd: [B, nbus], Qd: [B, nbus]
    Output:
    ...
    def __init__(self, nbus: int, ngen: int, nbranch: int, slack_imag_idx: int, hidden: int = 512):
        ...
        # ----------------------------------------------------
        # Branch 4: Lagrange Multipliers (Duals) NN
        # ----------------------------------------------------
        self.net_duals = nn.Sequential(
            nn.Linear(self.in_dim, hidden), nn.ReLU(),
            nn.Linear(hidden, hidden), nn.ReLU(),
            nn.Linear(hidden, hidden), nn.ReLU(),
            nn.Linear(hidden, self.num_duals)
        )
    ...
    def forward(self, Pd: torch.Tensor, Qd: torch.Tensor, problem: dict) -> tuple:
        ...
        duals_raw = self.net_duals(x)

        # ----------------------------------------------------
        # 2. Bound Primal Guesses (Initial Guess y_0)
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # 3. Slice Dual Variables (Lagrange Multipliers lambda, mu)
        # ----------------------------------------------------
        idx = 0
        lam_p = duals_raw[:, idx : idx+self.nbus]; idx += self.nbus
        lam_q = duals_raw[:, idx : idx+self.nbus]; idx += self.nbus

        mu_sf = duals_raw[:, idx : idx+self.nbranch]; idx += self.nbranch
        mu_st = duals_raw[:, idx : idx+self.nbranch]; idx += self.nbranch

        mu_ang_max = duals_raw[:, idx : idx+self.nbranch]; idx += self.nbranch
        mu_ang_min = duals_raw[:, idx : idx+self.nbranch]; idx += self.nbranch

        mu_v_max = duals_raw[:, idx : idx+self.nbus]; idx += self.nbus
        mu_v_min = duals_raw[:, idx : idx+self.nbus]; idx += self.nbus

        mu_pg_max = duals_raw[:, idx : idx+self.ngen]; idx += self.ngen
        mu_pg_min = duals_raw[:, idx : idx+self.ngen]; idx += self.ngen
        mu_qg_max = duals_raw[:, idx : idx+self.ngen]; idx += self.ngen
        mu_qg_min = duals_raw[:, idx : idx+self.ngen]; idx += self.ngen
...
if __name__ == "__main__":
    ...
    # *Updated Model Call*
    model_kkt = BranchedHardKKT_QCQPMLP(
        nbus=problem["nbus"],
        ngen=problem["ngen"],
    ...
    model_save_path = f"./model/best_hardkkt_MULTI_{case_name}_{epochs}epochs_{timestamp}.pth"

    # 5. Optimization Loop Execution
    print("\nBeginning execution of parallelized training matrix loops for Multi-Branch Hard KKT...")
