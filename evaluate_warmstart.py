#!/usr/bin/env python3
"""
NN-warm-started IPOPT ACOPF evaluation for reviewer feasibility-optimality-runtime analysis.

For each trained neural model/run and each held-out test instance:
  1) obtain the raw NN prediction (v, pg, qg),
  2) use the NN prediction as the initial point for the ordinary cost-minimizing ACOPF,
  3) solve with IPOPT and report convergence, feasible objective gap, warm-start
     solve time, total online time, and speedup versus the stored cold-start IPOPT run.
...
def build_sparse_acopf_model(problem_np, slack_imag_idx):
    """Build the ordinary cost-minimizing ACOPF using the same sparse AC equations as evaluate_new.py."""
    print("Building sparse Pyomo ACOPF model (this happens only once)...")
    ...
    # Ordinary ACOPF generation-cost objective.
    c2 = np.asarray(problem_np["c2"], dtype=float)
    c1 = np.asarray(problem_np["c1"], dtype=float)
    c0 = np.asarray(problem_np["c0"], dtype=float)

    def cost_rule(mm):
        return sum(float(c2[g]) * mm.pg[g]**2 + float(c1[g]) * mm.pg[g] + float(c0[g])
                   for g in range(ngen))

    m.cost = pyo.Objective(rule=cost_rule, sense=pyo.minimize)
    print("Sparse Pyomo ACOPF model built successfully.")
    return m
...
def configure_warm_start(m, Pd, Qd, v_pred, pg_pred, qg_pred, problem_np, slack_imag_idx):
    """
    Partial NN warm start:
      - Pg and Qg initialized from the NN prediction.
      - Voltage uses the standard flat start: Vr = 1, Vi = 0.
    """
    nbus = int(problem_np["nbus"])
    ngen = int(problem_np["ngen"])
    ...
    # Set load for this test instance
    for b in range(nbus):
        m.Pd[b] = float(Pd[b])
        m.Qd[b] = float(Qd[b])

    # NN warm start ONLY for Pg and Qg
    for g in range(ngen):
        m.pg[g].value = float(
            (pmin[g] + pmax[g]) / 2.0
        )
    ...
    # FLAT voltage start -- do NOT use NN voltage
    for b in range(nbus):
        m.v[b].value = 1.0          # Vr
        m.v[nbus + b].value = 0.0  # Vi

    # Reference-bus imaginary component
    m.v[int(slack_imag_idx)].value = 0.0

def main():
    parser = argparse.ArgumentParser(description="NN-warm-started IPOPT ACOPF evaluation.")
    ...
    cold_ipopt_time = np.asarray(gt_data["solve_time"][mask], dtype=np.float64)
    ...
    cold_ipopt_time = cold_ipopt_time[:n_eval]
    ...
    # Build the ordinary ACOPF once; only loads and initial values change.
    acopf_model = build_sparse_acopf_model(problem_np, slack_imag_idx)
    ...
    # We provide a primal NN initialization only (no dual/multiplier warm start).
    ...
    for arch_name, cfg in architectures.items():
        ...
        for run_idx, ckpt in enumerate(cfg["paths"], start=1):
            ...
            for i in range(n_eval):
                ...
                try:
                    results = solver.solve(acopf_model, tee=args.tee)
                ...
                if success:
                    v_rest = np.array([pyo.value(acopf_model.v[j]) for j in acopf_model.BUS2], dtype=float)
                    pg_rest = np.array([pyo.value(acopf_model.pg[g]) for g in acopf_model.GEN], dtype=float)
                    qg_rest = np.array([pyo.value(acopf_model.qg[g]) for g in acopf_model.GEN], dtype=float)
                    warm_cost = generation_cost_np(pg_rest, problem_np)
                    warm_gap = 100.0 * (warm_cost - ipopt_cost[i]) / ipopt_cost[i]

                    # Check the converged warm-started IPOPT solution with the same feasibility metrics.
                    with torch.no_grad():
                        ...
                        warm_max_eq = float(rv["max_eq"].item())
                        warm_mean_eq = float(rv["mean_eq"].item())
                        warm_max_ineq = float(rv["max_ineq"].item())
                        warm_mean_ineq = float(rv["mean_ineq"].item())
                else:
                    warm_cost = np.nan
                    warm_gap = np.nan
                    warm_max_eq = np.nan
                    warm_mean_eq = np.nan
                    warm_max_ineq = np.nan
                    warm_mean_ineq = np.nan
                ...
                rows.append({
                    ...
                    "Cold_IPOPT_Time_s": float(cold_ipopt_time[i]),
                    "Warm_IPOPT_Success": bool(success),
                    "Warm_IPOPT_Status": term,
                    "Warm_IPOPT_Time_s": float(warm_time),
                    "Total_Online_Time_s": float(inference_per_instance + warm_time),
                    "Speedup_vs_Cold_IPOPT": float(cold_ipopt_time[i] / (inference_per_instance + warm_time)) if (inference_per_instance + warm_time) > 0 else np.nan,
                    "Warm_IPOPT_Cost": warm_cost,
                    "IPOPT_Cost": float(ipopt_cost[i]),
                    "Warm_IPOPT_Obj_Gap_pct": warm_gap,
                    "Warm_IPOPT_Max_Eq": warm_max_eq,
                    "Warm_IPOPT_Mean_Eq": warm_mean_eq,
                    "Warm_IPOPT_Max_Ineq": warm_max_ineq,
                    "Warm_IPOPT_Mean_Ineq": warm_mean_ineq,
                })
    ...
    def summarize_run(g):
        ok = g["Warm_IPOPT_Success"].astype(bool)
        valid = g.loc[ok]
        return pd.Series({
            "N": len(g),
            "Warm_IPOPT_Success_Rate": ok.mean(),
            "Raw_Obj_Gap_Mean_pct": g["Raw_Obj_Gap_pct"].mean(),
            "Raw_Max_Eq_Mean": g["Raw_Max_Eq"].mean(),
            "Raw_Max_Ineq_Mean": g["Raw_Max_Ineq"].mean(),
            "Warm_IPOPT_Obj_Gap_Mean_pct": valid["Warm_IPOPT_Obj_Gap_pct"].mean(),
            "Warm_IPOPT_Abs_Obj_Gap_Mean_pct": valid["Warm_IPOPT_Obj_Gap_pct"].abs().mean(),
            "Warm_IPOPT_Max_Eq_Mean": valid["Warm_IPOPT_Max_Eq"].mean(),
            "Warm_IPOPT_Max_Ineq_Mean": valid["Warm_IPOPT_Max_Ineq"].mean(),
            "NN_Time_Mean_s": g["NN_Time_s"].mean(),
            "Cold_IPOPT_Time_Mean_s": g["Cold_IPOPT_Time_s"].mean(),
            "Warm_IPOPT_Time_Mean_s": g["Warm_IPOPT_Time_s"].mean(),
            "Total_Online_Time_Mean_s": g["Total_Online_Time_s"].mean(),
            "Speedup_vs_Cold_IPOPT_Mean": g["Speedup_vs_Cold_IPOPT"].mean(),
        })
    ...
    run_out = f"result/warmstart_runlevel_case{bus_number}.csv"
    run_summary.to_csv(run_out, index=False)

    # Final architecture summary = mean +/- sample SD across independent runs.
    metrics = [
        "Warm_IPOPT_Success_Rate",
        "Raw_Obj_Gap_Mean_pct",
        "Raw_Max_Eq_Mean",
        "Raw_Max_Ineq_Mean",
        "Warm_IPOPT_Obj_Gap_Mean_pct",
        "Warm_IPOPT_Abs_Obj_Gap_Mean_pct",
        "Warm_IPOPT_Max_Eq_Mean",
        "Warm_IPOPT_Max_Ineq_Mean",
        "NN_Time_Mean_s",
        "Cold_IPOPT_Time_Mean_s",
        "Warm_IPOPT_Time_Mean_s",
        "Total_Online_Time_Mean_s",
        "Speedup_vs_Cold_IPOPT_Mean",
    ]
    ...
    summary_out = f"result/warmstart_summary_case{bus_number}.csv"
    summary.to_csv(summary_out, index=False)

    print("\n============================================================")
    print("NN-WARM-STARTED IPOPT EVALUATION COMPLETE")
    print("============================================================")
