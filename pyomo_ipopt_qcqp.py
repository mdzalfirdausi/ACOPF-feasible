...
def build_acopf_model(problem_dict, slack_imag_idx):
    """
    Builds the Pyomo ACOPF model ONCE. 
    Uses mutable parameters for loads to allow instant hot-swapping.
    """
    print("Building Pyomo model (this happens only once)...")
    m = pyo.ConcreteModel()
    
    nbus = problem_dict["nbus"]
    ngen = problem_dict["ngen"]
    
    pmax, pmin = problem_dict["pmax"], problem_dict["pmin"]
    qmax, qmin = problem_dict["qmax"], problem_dict["qmin"]
    Vmax2, Vmin2 = problem_dict["Vmax"]**2, problem_dict["Vmin"]**2
    smax2 = problem_dict["smax"]**2
    
    # --- 1. MUTABLE PARAMETERS FOR LOAD ---
    ...
    # --- 3. Objective ---
    c2, c1, c0 = problem_dict["c2"], problem_dict["c1"], problem_dict["c0"]
    def cost_rule(m):
    ...
    for bus_i in range(nbus):
        # Active Power Balance (Now uses m.Pd)
        gen_p = 0.0
        for g in m.GEN:
            if C_g[bus_i, g] != 0:
    ...
    for br in range(nbranch):
        ...
        p_from, q_from = quad_form(problem_dict["M_pf"][br]), quad_form(problem_dict["M_qf"][br])
        m.Constraints.add(p_from**2 + q_from**2 <= smax_val)
        
        p_to, q_to = quad_form(problem_dict["M_pt"][br]), quad_form(problem_dict["M_qt"][br])
...
if __name__ == "__main__":
    ...
    pmax, pmin = problem_np["pmax"], problem_np["pmin"]
    qmax, qmin = problem_np["qmax"], problem_np["qmin"]
    nbus = problem_np["nbus"]
    ...
    for i in range(eval_limit):
        ...
        results = solver.solve(m, tee=tee_flag) 
        ...
        # Extract Results
        pg_res = np.array([pyo.value(m.pg[g]) for g in m.GEN])
        qg_res = np.array([pyo.value(m.qg[g]) for g in m.GEN])
        v_res = np.array([pyo.value(m.v[v_idx]) for v_idx in m.BUS2])
