...
class baselineQCQPMLP(nn.Module):
    ...
    Input:
        ...
        Qd: [B, nbus]
    Output:
        v:  [B, 2*nbus] (Rectangular voltages ONLY)
    """
    def __init__(self, nbus: int, ngen: int, slack_imag_idx: int, hidden: int = 512):
        ...
        # Core MLP Matrix Layer Sequence (Notice: output dimension is ONLY out_dim_v now!)
        self.net = nn.Sequential(
            ...
            nn.Linear(hidden, hidden),
            nn.ReLU(),
            nn.Linear(hidden, self.out_dim_v), # <-- CHANGED: No more generator dimensions
        )

    def forward(self, Pd: torch.Tensor, Qd: torch.Tensor, problem: dict) -> torch.Tensor:
        B = Pd.shape[0]
        x = torch.cat([Pd, Qd], dim=-1)
        v_raw = self.net(x) # <-- CHANGED: Direct voltage output

        # 1. Bound Voltages to [-Vmax, Vmax] using Tanh for smooth gradients
        ...
        # 2. Constraint (2m): Enforce slack imaginary part = 0 exactly
        v_clone = v.clone()
        v_clone[:, self.slack_imag_idx] = 0.0
        v = v_clone

        # <-- CHANGED: Return ONLY v. pg and qg will be calculated analytically in the loss!
        return v
...
def compute_qcqp_loss(model: nn.Module, Pd_batch: torch.Tensor, Qd_batch: torch.Tensor, problem: dict, weights: dict):
    ...
    # 1. Predict ONLY rectangular voltages
    v = model(Pd_batch, Qd_batch, problem)
    ...
    C_g = problem["C_g"] # Shape: [nbus, ngen]
    ...
    # Evaluate Quadratic Forms for Nodal Injections
    vp = quad_batch_stack(v, M_p) # Shape: [B, nbus]
    vq = quad_batch_stack(v, M_q)
    
    # =========================================================================
    # --- ARCHITECTURAL COMPLETION: ANALYTICAL GENERATION CALCULATION ---
    # =========================================================================
    # Find exact bus indices where each generator sits:
    gen_bus_indices = C_g.argmax(dim=0) 
    
    # Calculate required generation directly from Kirchhoff's Current Law: P_gen = V^2*Y + P_demand
    pg = vp[:, gen_bus_indices] + Pd_batch[:, gen_bus_indices]
    qg = vq[:, gen_bus_indices] + Qd_batch[:, gen_bus_indices]
    # =========================================================================

    # Evaluate Branch Flow & Voltage Security Quadratic Forms
    ...
    # Objective (Eq 2a) - Minimize Generation Fuel Cost
    ...
    # Nodal Power Balance Residuals
    h_p = (pg @ C_g.T) - Pd_batch - vp
    h_q = (qg @ C_g.T) - Qd_batch - vq
    
    # Generator Bound Violations (Must be penalized now since they aren't hard-bounded by Sigmoid!)
    ...
    g_v_max = vv - (Vmax**2); g_v_min = (Vmin**2) - vv
