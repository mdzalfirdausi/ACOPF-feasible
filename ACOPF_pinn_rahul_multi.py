...
class BranchedRahulPINN_Smax(nn.Module):
    """
    Version 3: Multi-Branch Neural Network for Rahul PINN (Original Paper Architecture).
    Uses three independent MLPs for:
      1. Voltages (NN_V)
      2. Generation (NN_G)
    ...
    def __init__(self, nbus, ngen, nbranch, hidden_dim=512):
        ...
        # ----------------------------------------------------
        # Branch 1: Voltage Variables (NN_V)
        # ----------------------------------------------------
        self.dim_v = 2 * nbus
        self.net_v = nn.Sequential(
            ...
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, self.dim_v)
        )

        # ----------------------------------------------------
        # Branch 2: Generation Variables (NN_G)
        # ----------------------------------------------------
        ...
        self.net_g = nn.Sequential(
            ...
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, self.dim_g)
        )

        # ----------------------------------------------------
        # Branch 3: Dual Variables / Multipliers (NN_L)
        # ----------------------------------------------------
        self.num_duals = (4 * nbus) + (4 * nbranch) + (4 * ngen)
        self.net_dual = nn.Sequential(
            nn.Linear(in_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, self.num_duals)
        )
    ...
    def forward(self, Pd, Qd):
        ...
        # ----------------------------------------------------
        # 1. Independent Forward Passes
        # ----------------------------------------------------
        v = self.net_v(x)
        pq = self.net_g(x)
        duals = self.net_dual(x)
        
        # ----------------------------------------------------
        # 2. Slice Primal Generation Variables
        # ----------------------------------------------------
        pg = pq[:, :self.ngen]
        qg = pq[:, self.ngen:]
        
        # ----------------------------------------------------
        # 3. Slice Dual Variables (Lagrange Multipliers)
        # ----------------------------------------------------
        idx = 0
        ...
        mu_qg_min = duals[:, idx : idx+self.ngen]; idx += self.ngen
        
        # Return exact same signature expected by the loss function
...
if __name__ == "__main__":
    ...
    model_rahul = BranchedRahulPINN_Smax(
        nbus=problem["nbus"],
        ngen=problem["ngen"],
