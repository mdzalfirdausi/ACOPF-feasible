...
# --- DIAGNOSTIC NAN TRACKER ---
print("\n--- Running Inter-Loop NaN Diagnostics ---")
vars_to_check = {
    "baseMVA": baseMVA, "Z_sq": Z_sq, "g": g, "b": b, 
    "tau": tau, "Gs": Gs, "Bs": Bs, "g11": g11, "b11": b11
}

for name, var in vars_to_check.items():
    if isinstance(var, np.ndarray):
        if np.isnan(var).any():
            nan_indices = np.where(np.isnan(var))[0]
            print(f"  !! Found NaN inside array '{name}' at index/indices: {nan_indices}")
    else:
        if np.isnan(var):
            print(f"  !! '{name}' scalar is NaN!")
